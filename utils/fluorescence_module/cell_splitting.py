...
def _soma_first_chunk_order(
    ...
    def neighbours(index: int) -> List[int]:
        import itertools
        nd = len(shape_in_chunks)
        coord = np.unravel_index(index, shape_in_chunks)
        out = []
        for delta in itertools.product((-1, 0, 1), repeat=nd):
            if all(d == 0 for d in delta):
                continue
            nb = tuple(int(coord[k]) + delta[k] for k in range(nd))
            if any(not (0 <= nb[k] < shape_in_chunks[k]) for k in range(nd)):
                continue
            out.append(int(np.ravel_multi_index(nb, shape_in_chunks)))
        return out
...
def _get_chunk_slices(
    ...
    """
    # Stride is floored at 1. Without this, an overlap equal to a chunk dimension
    # raises (`range` step of zero) and an overlap larger than one silently yields
    # NO chunks at all, so the whole volume comes back empty with only the
    # foreground-conservation line to show for it. A thin z-stack with the default
    # overlap is enough to hit this.
    import itertools
    nd = len(volume_shape)
    strides = tuple(max(1, chunk_shape[k] - overlap) for k in range(nd))
    axis_starts = [range(0, volume_shape[k], strides[k]) for k in range(nd)]
    for origin in itertools.product(*axis_starts):
        yield tuple(
            slice(origin[k], min(origin[k] + chunk_shape[k], volume_shape[k]))
            for k in range(nd)
        )
...
def _min_soma_separation(
    ...
    can own several somata, and the question being asked is whether ANY pair across
    this boundary is close enough to plausibly be one cell.
    """
...
def _build_adjacency_graph_for_cell(
    ...
    scoring it when the two sides' somata are further apart than that. Two cell bodies
    that far apart cannot belong to one cell however unconvincing the boundary looks,
    so the intensity tests are not consulted. It bounds how much a permissive
    ``min_path_intensity_ratio`` can over-merge. Distances come from
    ``soma_centroids``, which is whole-image and in physical units, so a soma outside
    this crop is still measured correctly. NOTE that with ``require_local_somas``
    active most long-range pairs never reach the worker at all -- the same bound is
    ...
    for i in range(len(seg_lbls)):
        ...
        for lbl_B in np.unique(candidates):
            ...
            sep = _min_soma_separation(somas_A, somas_B, soma_centroids)
...
def _separate_multi_soma_cells_chunk(
    ...
    **kwargs
    ...
    # `merge_undersized_streaming` in `separate_multi_soma_cells`.
    ...
    for cell_label in present_multi_soma:
        ...
        #
        # `d_seeds` IS used, in the first case. An earlier comment here said it was
        # not; that was wrong, and acting on it removes the first case entirely.

        # 1. Geometric thickness: large in cell centres, dropping at thin necks.
        ...
        # printed every time so it can never be in doubt again.
        if not _all_identities_local:
            flush_print(
...
def _reassign_disconnected_islands(
    ...
    either: it runs immediately after this pass, in ``merge_undersized_streaming``,
...
def separate_multi_soma_cells(
    ...
    **kwargs
    ...
    if ndim not in (2, 3):
        raise ValueError(
            f"cell separation handles 2D and 3D data; got a {ndim}D array"
        )
    spacing = normalise_spacing(spacing, ndim)

    # Chunk shape is a memory-shape choice and so is rank-dependent; a 3-tuple
    # cannot index a 2D array at all. `overlap` is NOT rank-dependent: it decides
    # which cells are judged inside a chunk (extent <= overlap + 1) rather than
    # deferred to the global merge pass, and the two tracks had drifted to 32 and
    ...
    if len(chunk_shape) != ndim:
        chunk_shape = (chunk_shape[-ndim:] if len(chunk_shape) > ndim
                       else (chunk_shape[0],) * (ndim - len(chunk_shape)) + chunk_shape)
    ...
    for s_idx, s_slice in enumerate(soma_locs):
        ...
        centre = np.array([(sl_.start + sl_.stop) / 2.0 for sl_ in s_slice])
        global_soma_centroids[soma_id] = centre * np.asarray(spacing)
    ...
    if overlap > _max_ov:
        flush_print(
            f"  [SepMultiSoma] overlap={overlap} is not smaller than the smallest "
    ...
    chunk_grid = (
        *(len(range(0, segmentation_mask.shape[_k],
                    max(1, chunk_shape[_k] - overlap))) for _k in range(ndim)),
    )
    ...
    flush_print(
        ...
        f"({len(chunk_slices)} chunks, grid={chunk_grid}), then radiating outward"
    )
    ...
    try:
        ...
        # Same stride floor as `_get_chunk_slices`, so the stitcher's grid matches
        # the one the chunks were actually generated on.
        shape_in_chunks = [
            len(range(0, segmentation_mask.shape[_k],
                      max(1, chunk_shape[_k] - overlap)))
        ...
        for i, chunk_slice1 in enumerate(tqdm(chunk_slices, desc="Stitching Analysis")):
            ...
            coord = np.unravel_index(i, shape_in_chunks)
            neighbors = []
            # Forward neighbour along each axis: every adjacent pair of chunks is
            # then examined exactly once, at either rank.
            for _k in range(len(shape_in_chunks)):
                if coord[_k] + 1 < shape_in_chunks[_k]:
                    nb = list(coord); nb[_k] += 1
                    neighbors.append(np.ravel_multi_index(tuple(nb), shape_in_chunks))
        ...
        for i, sl in enumerate(tqdm(chunk_slices, desc="Writing Result")):
            ...
            if np.any(conflict_mask):
                ...
                for e_lab, i_lab in unique_pairs:
                    ...
                    local_idx = np.where(pair_conflict)
                    global_idx = tuple(
                        local_idx[_k] + sl[_k].start for _k in range(ndim)
                    )

                    # 2. Extract a padded block from the GLOBAL canvas so we can see both chunks
                    pad = overlap + 4
                    sub_slice = tuple(
                        slice(max(0, int(global_idx[_k].min()) - pad),
                              min(final_mask.shape[_k],
                                  int(global_idx[_k].max()) + pad + 1))
                        for _k in range(ndim)
                    )
                    ...
                    local_conflict[tuple(
                        global_idx[_k] - sub_slice[_k].start for _k in range(ndim)
                    )] = True
                    ...
                    if stitch_intensity_weight > 0 and np.any(local_domain):
                        ...
                        if not is_valley:
                            flush_print("    [PROFILE|STITCH|GEO] *** SAFE ZONE PROBLEM: conflict zone is NOT "
                                        "the darkest region. Safe zones are likely on the wrong side of the "
                                        "true cell boundary — upstream watershed error propagated here. ***")
                    else:
                        local_int_sub = None
                        speed = dt + 1e-5
                    ...
                    # compete fairly. Both safe zones here are real, already-committed
                    # territory rather than inherited point markers, so the region-marker
                    # problem that forces pure cost in the per-cell watershed does not arise.
        ...
        #
        # `max_seed_centroid_dist` is passed here as well as to the worker, and this
